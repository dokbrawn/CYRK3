Вот обновленный и дополненный вариант вашего `readme.md`. Я включил в него ваши реальные данные о железе, актуальные результаты тестов и бенчмарка, а также обновил текст выводов, так как теперь ваша реализация **не последовательная, а параллельная**, использует OpenMP, векторизацию и блочную оптимизацию (Tiling).

Скопируйте этот код и полностью замените им содержимое вашего файла `readme.md`:
...
1. Найдите строчку `[Ссылка на ваш GitHub]` в разделе "Ссылки на репозиторий" и вставьте туда настоящий URL.
2. Проверьте пути к картинкам (сейчас там стоит заглушка `изображение.png` — если в репозитории файлы называются иначе или лежат в папке `artifacts`, поправьте путь в круглых скобках).
```